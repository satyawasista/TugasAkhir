...
@section('breadcrumc')
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Deta Rekam Medis Persalinan</li>
      </ol>
    </nav>
@endsection
@section('content')
<div class="container">
        ...
        <div class="card">
          <div class="card-body">
            <form class="row g-3" action="" method="GET">
              <div class="col-md-3">
              <input type="search" name="searchRMP" id="searchRMP" class="form-control" aria-describedby="passwordHelpInline" placeholder="Cari data Rekam Medis" autofocus="true" value="{{ $searchRMP }}" >
              </div>
            ...
            <table id="example2" class="table">
              ...
              <tr>
                <th scope="col">No</th>
                <th scope="col">@sortablelink('id_maternity_patient','Nama')</th>
                <th scope="col">@sortablelink('delivery_room','Masuk Kamar Bersalin')</th>
              ...
              <tbody>
                @php
                  $no =1;   
                @endphp
        @foreach ($data as $index => $bersalin)
        <tr>
          <th scope="row">{{$index + $data-> firstItem() }}</th>
            <td>{{ $bersalin->maternityPatient->patient->name }}</td>
   ...
   {{-- <script>
     $('.delete').click(function(){
       var birthRecordid = $(this).attr('data-id');
     ...
     .then((willDelete) => {
       if (willDelete) {
         window.location = "/hapusdatarekammedisbersalin/"+birthRecordid+""
